...
module HelperModule
  ...
  def unknown_collaborator_hash(user_name)
...

    {
      ...
      body: <<~EOF
        ...
        We have a process to manage github collaborators in code: #{GH_ORG_URL}/github-outside-collaborators
  ...
  def review_date_expires_soon_hash(user_name)
...

    {
      ...
      body: <<~EOF
        ...
        The user @#{user_name} has its access for this repository maintained in code here: #{GH_ORG_URL}/github-outside-collaborators
  ...
  # Query the everyone team and return its repositories
  #
  ...
  def get_everyone_team_repositories
    ...
    url = "https://api.github.com/orgs/#{ORG}/teams/everyone/repos?per_page=100"
    team_repositories = []
    json = GithubCollaborators::HttpClient.new.fetch_json(url)
    JSON.parse(json)
  ...
  # Get the github-outside-collaborators repository pull requests
  #
  ...
  # Create a pull request on the github-outside-collaborators repository
  #
  ...
  def create_pull_request(hash_body)
    ...
    if ENV.fetch("REALLY_POST_TO_GH", 0) == "1"
      url = "#{GH_API_URL}/github-outside-collaborators/pulls"
  ...
  # Create a GraphQL query that returns the github-outside-collaborators repository pull requests
  #
  # @return [String] the GraphQL query
  def pull_request_query
    %[
      {
        organization(login: "#{ORG}") {
          repository(name: "github-outside-collaborators") {
