...
class GithubCollaborators
  ...
  class FullOrgMember
    ...
    # Add repository names to the everyone team repositories array
    #
    # @param repositories [Array<String>] the repository names
    def add_everyone_team_repositories(repositories)
      logger.debug "add_everyone_team_repositories"
      @everyone_team_repositories = repositories
      @everyone_team_repositories.sort!
      @everyone_team_repositories.uniq!
    end
    ...
    def add_terraform_repositories(terraform_repositories)
      ...
      terraform_repositories.each do |terraform_repository_name|
        # Ignore excluded repositories ie the everyone team repositories and archived repositories
        # This is to focus on active repositories that should be tracked
        if !@everyone_team_repositories.include?(terraform_repository_name.downcase) && !@github_archived_repositories.include?(terraform_repository_name.downcase)
    ...
    def odd_full_org_member_check
      ...
      if (@github_repositories.length == 0 || @terraform_repositories.length == 0) && @everyone_team_repositories.length == 0
        return true
      end
      false
    end
    ...
    def get_full_org_member_repositories
      ...
      repositories.each do |repo|
        # Accept only moj-analytical-services repositories
    ...
    # This includes the everyone team and archived array of repository names
    # The reason for this is to focus on active repositories that should be tracked and filter
    # out those that do not need to be
    #
    # @param repository_name [String] the repository name
    # @return [Bool] true if the repository name is in the archived array or everyone team array of repositories
    def is_repo_already_known(repository_name)
      if @everyone_team_repositories.include?(repository_name) || @github_archived_repositories.include?(repository_name)
        return true
      end
      false
    end
